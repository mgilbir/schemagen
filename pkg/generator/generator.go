...
// overrides it: its title under Go naming rules, or "Root".
//
// Exported because the name is decided here but has to be predictable outside:
// the CLI compares the root names of a --shared-types run's inputs to tell a
// genuine duplicate-root-name collision from one caused by the input order, and
// a second implementation of this rule would drift out of agreement with the
// one that actually names the type.
func DefaultRootTypeName(s *schema.Schema) string {
	if s != nil && s.Title != "" {
...
// was already materialized in the package, either by an earlier schema's own
// root or by an earlier schema's $ref into this one.
//
// Typed rather than a bare fmt.Errorf because those two causes need different
// advice and only the caller knows which one applies: it holds the input order
// and the refs between the documents. The message here is the one for the first
// cause, and stands when the caller cannot establish the second.
type RootTypeCollisionError struct {
	Name string
}

func (e *RootTypeCollisionError) Error() string {
	return fmt.Sprintf("root type %q was already generated by an earlier schema in this package; give each schema a distinct root name", e.Name)
}
...
//
// Ref is kept because it is the only part of the reference that locates the
// problem. Two references into the same document, one to its root and one to
// `#/properties/k`, are the same Document and the same Package and are not the
// same reference at all: the first is ordinarily fine and the second is what
// fails. Reporting the document alone named the half that was working -- and
// said it was not generated while that run's own output held the generated file
// on disk. Issue #310.
type crossPackageMiss struct {
	Package  string
	Document string
	// Ref is the $ref exactly as the schema wrote it, fragment included.
	Ref string
	// DocumentGenerated distinguishes the two causes that arrive here. True: the
	// owning package generated this document, and it is *this node* that has no
	// Go declaration of its own -- a subschema the owner emits as a field, which
	// the README describes as an error rather than a copy. False: no type from
	// this document was ever registered, so the owning package did not generate
	// it in this run. The two need opposite advice, and the message that served
	// both gave the second one's.
	DocumentGenerated bool
}

// CrossPackageMissError reports $ref targets that belong to another package of
// a cross-package run but have no Go type there to import.
type CrossPackageMissError struct {
	Misses []string
}

func (e *CrossPackageMissError) Error() string {
	return fmt.Sprintf("cross-package $ref cannot be imported: %s", strings.Join(e.Misses, "; "))
}
...
// describe says which of the two causes occurred, and what to do about it.
func (m crossPackageMiss) describe() string {
	ref := m.Ref
	if ref == "" {
		ref = m.Document
	}
	if m.DocumentGenerated {
		return fmt.Sprintf("$ref %q resolves to a node of %q that its owning package %q gives no Go type of its own, "+
			"so there is nothing to import -- that package emits the node as part of another type and never as a "+
			"declaration; reference a named definition, such as a $defs entry, or add one to that document",
			ref, m.Document, m.Package)
	}
	return fmt.Sprintf("$ref %q resolves into %q, which this run assigns to package %q, and that package generated "+
		"no type from it; list that document among the inputs, or drop the --schema-package mapping that claims it",
		ref, m.Document, m.Package)
}
...
//
// ref is the reference as the schema wrote it, carried only so that a miss can
// name the fragment that failed rather than the document that did not: see
// crossPackageMiss. Pass "" where the position has no reference string of its
// own; the document is then the best the message can do.
func (g *Generator) foreignTypeFor(resolved *schema.Schema, ref string) (*NamedType, bool) {
	...
	if !ok || qt.ImportPath != pkg {
		...
		// Generate report it.
		document := documentIdentityOf(resolved)
		g.crossPackageMisses[crossPackageMiss{
			Package:           pkg,
			Document:          document,
			Ref:               ref,
			DocumentGenerated: reg.documentWasGenerated(document),
		}] = true
		return nil, false
	}
