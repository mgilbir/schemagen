// Code generated by schemagen. DO NOT EDIT.

package testpkg

import (
	"encoding/json"
	"fmt"
)

type Measurement struct {
	Count                int64                      `json:"count"`
	Rating               *json.Number               `json:"rating,omitempty"`
	Temperature          json.Number                `json:"temperature"`
	AdditionalProperties map[string]json.RawMessage `json:"-"`
	_jsonKeys            map[string]bool            // set by UnmarshalJSON for optional field / dependentSchemas validation
}

func (m *Measurement) UnmarshalJSON(data []byte) error {
	m.AdditionalProperties = nil
	m._jsonKeys = nil
	if string(data) == "null" {
		return fmt.Errorf("null is not allowed for type Measurement")
	}
	// The decode below is handed the document cut down to the properties this
	// schema declares, because encoding/json matches a key that matches no field
	// exactly a second time case-insensitively, and would fill "name" from a
	// "NAME" the schema never gave it. See jsonExactProperties and issue #245.
	//
	// The object is parsed once here and read again by the blocks below, so this
	// costs no parse that was not already being paid. Its error is held rather
	// than returned, so that a document which is not an object is still refused
	// by the decode that always refused it, in the words it always used.
	var raw map[string]json.RawMessage
	_rawErr := json.Unmarshal(data, &raw)
	_decodeData := data
	if _rawErr == nil {
		if _exact := jsonExactProperties(raw,
			"count",
			"rating",
			"temperature",
		); _exact != nil {
			_decodeData = _exact
		}
	}
	type Alias Measurement
	aux := &struct {
		*Alias
		Count       *jsonInteger `json:"count"`
		Rating      **jsonNumber `json:"rating"`
		Temperature *jsonNumber  `json:"temperature"`
	}{
		Alias: (*Alias)(m),
	}

	if err := json.Unmarshal(_decodeData, aux); err != nil {
		return err
	}

	// A number written 1.0 is the integer 1 from draft 6 on, and the shadows
	// above are what let encoding/json see it. Each outer pointer is nil when
	// the property was absent or null, both of which leave the field as it was.

	// A number held exactly is a json.Number, which is a string underneath and
	// would take a JSON string for a number; the shadows above are what refuse
	// one. Each outer pointer is nil when the property was absent or null, both
	// of which leave the field as it was.
	if aux.Count != nil {
		_iv := *aux.Count
		m.Count = int64(_iv)
	}
	if aux.Rating != nil {
		_iv := *aux.Rating
		m.Rating = jsonIntegerPtr(_iv, func(_ix0 jsonNumber) json.Number { return json.Number(_ix0) })
	}
	if aux.Temperature != nil {
		_iv := *aux.Temperature
		m.Temperature = json.Number(_iv)
	}
	{
		if _rawErr != nil {
			return _rawErr
		}
		// A property the schema gives a type to may not be written as null. By
		// the time the decode above has run there is nothing left to see: a null
		// leaves a nil pointer, a nil collection, or a scalar at its zero, which
		// is exactly what an absent property leaves, so the verdict has to be
		// taken from the document's own keys. See jsonNullRule for the nested
		// spelling of the same rule.
		for _, _nullKey := range []string{
			"count",
			"rating",
			"temperature",
		} {
			if _v, ok := raw[_nullKey]; ok && string(_v) == "null" {
				return fmt.Errorf("%s: null is not allowed", _nullKey)
			}
		}
		m._jsonKeys = make(map[string]bool, len(raw))
		for _k := range raw {
			m._jsonKeys[_k] = true
		}
		knownFields := map[string]bool{
			"count":       true,
			"rating":      true,
			"temperature": true,
		}
		for rawKey, rawVal := range raw {
			if knownFields[rawKey] {
				continue
			}
			if m.AdditionalProperties == nil {
				m.AdditionalProperties = make(map[string]json.RawMessage)
			}
			m.AdditionalProperties[rawKey] = rawVal
		}
	}

	return nil
}
func (m Measurement) MarshalJSON() ([]byte, error) {
	type Alias Measurement
	aux := struct {
		Alias
	}{
		Alias: (Alias)(m),
	}
	data, err := json.Marshal(aux)
	if err != nil {
		return nil, err
	}
	var obj map[string]json.RawMessage
	if err := json.Unmarshal(data, &obj); err != nil {
		return nil, err
	}
	for k, v := range m.AdditionalProperties {
		obj[k] = v
	}
	return json.Marshal(obj)
}

// Validate checks Measurement against its JSON Schema constraints.
func (m Measurement) Validate() error {
	// Required properties must be present in the source JSON. _jsonKeys is
	// populated by UnmarshalJSON; when nil (the value was not built from JSON)
	// presence is untracked and the check is skipped, consistent with how
	// optional-property validation below treats _jsonKeys.
	if m._jsonKeys != nil {
		for _, _req := range []string{"count", "temperature"} {
			if !m._jsonKeys[_req] {
				return fmt.Errorf("%s: required property is missing", _req)
			}
		}
	}
	if m.Count%5 != 0 {
		return fmt.Errorf("count: value %v is not a multiple of 5", m.Count)
	}
	if m._jsonKeys["rating"] {
		if m.Rating != nil && jsonNumberCmp(json.Number(*m.Rating), "0") < 0 {
			return fmt.Errorf("rating: value %v is less than minimum 0", *m.Rating)
		}
	}
	if m._jsonKeys["rating"] {
		if m.Rating != nil && jsonNumberCmp(json.Number(*m.Rating), "10") > 0 {
			return fmt.Errorf("rating: value %v exceeds maximum 10", *m.Rating)
		}
	}
	if m._jsonKeys["rating"] {
		if m.Rating != nil && !jsonNumberIsMultipleOf(json.Number(*m.Rating), "0.5") {
			return fmt.Errorf("rating: value %v is not a multiple of 0.5", *m.Rating)
		}
	}
	if jsonNumberCmp(json.Number(m.Temperature), "-273.15") <= 0 {
		return fmt.Errorf("temperature: value %v must be greater than -273.15", m.Temperature)
	}
	if jsonNumberCmp(json.Number(m.Temperature), "1000") >= 0 {
		return fmt.Errorf("temperature: value %v must be less than 1000", m.Temperature)
	}
	return nil
}
