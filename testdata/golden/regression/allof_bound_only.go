...
func (a *AllOfBoundOnly) UnmarshalJSON(data []byte) error {
...

	{
		...
		if len(oneofData) > 0 && string(oneofData) != "null" {
			...
			if oneofMatched == 0 {
				return fmt.Errorf("AllOfBoundOnly.Union: no matching oneOf variant: %w", oneofLastErr)
			}
			if oneofMatched > 1 && oneofOpaque == 0 {
				...
				case oneofStrictErr != nil:
					...
					return fmt.Errorf("AllOfBoundOnly.Union: no matching oneOf variant: %w", oneofStrictErr)
				}
			}
			if oneofMatched > 1 {
				return fmt.Errorf("AllOfBoundOnly.Union: multiple oneOf variants matched (%d), expected exactly 1", oneofMatched)
			}
		}
	}
	{
