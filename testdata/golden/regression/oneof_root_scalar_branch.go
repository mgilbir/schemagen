...
func (o *OneOfRootScalarBranchValueOption0) UnmarshalJSON(data []byte) error {
	...
	o._jsonKeys = nil
	if string(data) == "null" {
		return fmt.Errorf("null is not allowed for type OneOfRootScalarBranchValueOption0")
	}
...
func (o *OneOfRootScalarBranch) UnmarshalJSON(data []byte) error {
...

	{
		...
		if len(oneofData) > 0 && string(oneofData) != "null" {
			...
			if oneofMatched == 0 {
				return fmt.Errorf("OneOfRootScalarBranch.Value: no matching oneOf variant: %w", oneofLastErr)
			}
			if oneofMatched > 1 && oneofOpaque == 0 {
				...
				case oneofStrictErr != nil:
					...
					return fmt.Errorf("OneOfRootScalarBranch.Value: no matching oneOf variant: %w", oneofStrictErr)
				}
			}
			if oneofMatched > 1 {
				return fmt.Errorf("OneOfRootScalarBranch.Value: multiple oneOf variants matched (%d), expected exactly 1", oneofMatched)
			}
		}
	}
